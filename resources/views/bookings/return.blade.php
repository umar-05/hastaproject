    ...
    <div class="min-h-screen bg-white py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            @if ($errors->any())
                <div class="mb-8 bg-red-50 border-l-4 border-red-500 p-4 rounded-r shadow-md">
                    <div class="flex">
                        ...
                        <div class="ml-3"><h3 class="text-sm leading-5 font-bold text-red-800">Please fix the following errors:</h3><ul class="list-disc list-inside text-sm text-red-700 mt-1">@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>
                    </div>
                </div>
            @endif
            ...
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-3xl font-black text-hasta-red tracking-tight">Car Return Form</h1>
            ...
            <form id="inspectionForm" action="{{ route('bookings.store-inspection', $booking->bookingID) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="bg-gray-50 rounded-2xl p-8 mb-8 border border-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                        ...
                        <div class="col-span-1 md:col-span-2"><p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Vehicle</p><p class="font-bold text-lg text-gray-900">{{ $booking->fleet->modelName ?? 'Vehicle' }} {{ $booking->fleet->year ?? '' }}</p></div>
                    </div>
                </div>

                {{-- Date & Time (Return) --}}
                <div class="flex flex-col md:flex-row gap-4 mb-10">
                    <div class="flex-1 bg-gray-100 rounded-xl px-6 py-4 flex items-center gap-4 border-l-4 border-hasta-red"><svg class="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg><span class="font-bold text-gray-800 text-lg">{{ \Carbon\Carbon::parse($booking->returnDate)->format('D, d M Y') }}</span></div>
                    <div class="flex-1 bg-gray-100 rounded-xl px-6 py-4 flex items-center gap-4 border-l-4 border-gray-300"><svg class="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span class="font-bold text-gray-800 text-lg">{{ \Carbon\Carbon::parse($booking->returnDate)->format('h : i A') }}</span></div>
                </div>
                ...
                <div class="mb-12">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        @php $photoMap = ['Front' => 'frontViewImage', 'Back' => 'backViewImage', 'Left' => 'leftViewImage', 'Right' => 'rightViewImage']; @endphp
                        @foreach(['Front', 'Back', 'Left', 'Right'] as $angle)
                            @php $dbColumn = $photoMap[$angle]; $existingPhoto = $inspection->$dbColumn ?? null; @endphp
                            <div class="relative group image-upload-container h-32 w-full">
                                <div class="border-2 border-dashed border-gray-200 rounded-2xl h-full w-full flex flex-col items-center justify-center text-gray-400 group-hover:border-hasta-red group-hover:text-hasta-red transition bg-gray-50 cursor-pointer overflow-hidden placeholder-content {{ $existingPhoto ? 'hidden' : '' }}">
                                    <svg class="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg><span class="text-xs font-bold uppercase tracking-wider">{{ $angle }} Image</span>
                                </div>
                                ...
                                <div class="absolute inset-0 z-0 image-preview {{ $existingPhoto ? '' : 'hidden' }} h-full w-full rounded-2xl overflow-hidden bg-white border border-gray-200">
                                    @if($existingPhoto)<img src="{{ asset('storage/' . $existingPhoto) }}" class="w-full h-full object-cover">@endif
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
                    <div>
                        <h3 class="text-lg font-bold mb-4 text-gray-900">Final Fuel Level</h3>
                        <div class="relative group image-upload-container h-40">
                            ...
                            <div class="border-2 border-dashed border-gray-200 rounded-2xl h-full flex flex-col items-center justify-center text-gray-400 bg-gray-50 transition group-hover:border-hasta-red placeholder-content {{ $fuelPhoto ? 'hidden' : '' }}">
                                <svg class="w-10 h-10 mb-2 group-hover:text-hasta-red" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg><span class="text-sm font-medium">Click to upload fuel image</span>
                            </div>
                            ...
                            <div class="absolute inset-0 z-0 image-preview {{ $fuelPhoto ? '' : 'hidden' }} h-full w-full rounded-2xl overflow-hidden bg-white border border-gray-200">
                                @if($fuelPhoto)<img src="{{ asset('storage/' . $fuelPhoto) }}" class="w-full h-full object-cover">@endif
                            </div>
                        </div>
                        <div class="mt-4 flex items-center gap-3">
                            ...
                            <select name="fuel_level" class="border-gray-200 bg-gray-50 rounded-lg text-sm focus:ring-hasta-red focus:border-hasta-red">
                                @foreach(['1', '2', '3', '5' => 'Half', '10' => 'Full'] as $val => $label)
                                    @php $value = is_string($val) ? $val : $label; $displayText = is_string($val) ? $label : $label . ' Bar' . ($label > 1 ? 's' : ''); $selected = (isset($inspection->fuelBar) && $inspection->fuelBar == $value) ? 'selected' : ''; @endphp
                    ...
                    <div>
                        <h3 class="text-lg font-bold mb-4 text-gray-900">Final Mileage</h3>
                        <div class="flex items-center gap-3">
                            <input type="number" name="mileage" step="0.1" value="{{ $inspection->mileage ?? '' }}" class="border-gray-200 bg-gray-50 rounded-xl px-4 py-3 w-full text-lg font-bold focus:ring-hasta-red focus:border-hasta-red" placeholder="e.g. 54500">
                ...
                <div class="mb-10">
                    ...
                    <div class="relative w-full">
                        @if(isset($inspection->signature))
                            <div class="border border-gray-200 rounded-xl w-full h-48 bg-white shadow-inner flex items-center justify-center relative"><img src="{{ asset('storage/' . $inspection->signature) }}" class="h-full object-contain"><div class="absolute top-4 right-4 bg-green-100 text-green-800 text-xs font-bold px-3 py-1.5 rounded-lg">Signed</div></div>
                        @else
                ...
                <div class="flex gap-4 pt-4 border-t border-gray-100">
                    <a href="{{ route('bookings.show', $booking->bookingID) }}" class="flex-1 border border-gray-300 bg-white text-gray-700 font-bold py-4 rounded-xl text-center hover:bg-gray-50 transition">Cancel</a>
                    @if(!isset($inspection))
                    <button type="button" onclick="submitForm()" class="flex-1 bg-hasta-red text-white font-bold py-4 rounded-xl hover:bg-red-700 transition flex items-center justify-center gap-2 shadow-lg shadow-red-200"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>Submit Return Form</button>
                    @else
                    <button type="button" disabled class="flex-1 bg-gray-300 text-gray-500 font-bold py-4 rounded-xl cursor-not-allowed">Already Submitted</button>
                    @endif
                </div>
            </form>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fileInputs.forEach(input => {
                input.addEventListener('change', function(e) {
                    ...
                    if (file) {
                        ...
                        reader.onload = function(e) { placeholder.classList.add('hidden'); preview.classList.remove('hidden'); preview.innerHTML = `<img src="${e.target.result}" class="w-full h-full object-cover">`; }
                        reader.readAsDataURL(file);
                    } else { placeholder.classList.remove('hidden'); preview.classList.add('hidden'); preview.innerHTML = ''; }
                });
            });
            ...
            if (canvas) {
                ...
                function getPointerPos(event) { const rect = canvas.getBoundingClientRect(); const clientX = event.touches ? event.touches[0].clientX : event.clientX; const clientY = event.touches ? event.touches[0].clientY : event.clientY; return { x: clientX - rect.left, y: clientY - rect.top }; }
        ...
        function submitForm() {
            ...
            if (canvas) { signatureInput.value = canvas.toDataURL('image/png'); }
            form.submit();
        }
    </script>
</x-app-layout>