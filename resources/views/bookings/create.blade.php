...
<x-app-layout>
    ...
    @php
        $vehicleImage = 'default-car.png'; // Fallback
        
        if (isset($car)) {
            // If specific photo is saved in DB, use it
            if (!empty($car->photos)) {
                $vehicleImage = $car->photos;
            } else {
                // Otherwise, determine based on Model Name and Year
                ...
                if (str_contains($model, 'axia')) {
                    ...
                    $vehicleImage = 'default-suv.png'; 
                } elseif (str_contains($model, 'y15')) {
                    $vehicleImage = 'default-bike.png';
                }
            }
        }
    @endphp
