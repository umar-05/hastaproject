    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-start mb-8">
                <div>
                    <h1 class="text-4xl font-black text-[#1A1C1E]">Missions</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-6">
                    ...
                    <div>
                        <span class="text-3xl font-black text-gray-900">{{ $missions->where('status', 'Available')->count() }}</span>
                ...
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-6">
                    ...
                    <div>
                        <span class="text-3xl font-black text-gray-900">{{ $missions->where('status', 'Ongoing')->count() }}</span>
                ...
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-6">
                    ...
                    <div>
                        <span class="text-3xl font-black text-gray-900">{{ $missions->where('status', 'Completed')->count() }}</span>
                ...
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-6">
                    ...
                    <div>
                        <span class="text-3xl font-black text-gray-900">RM {{ number_format($missions->where('status', 'Completed')->where('assigned_to', auth()->user()->staffID)->sum('commission'), 2) }}</span>
            ...
            <div class="flex gap-4 mb-10">
                ...
                <a href="{{ route('staff.missions.index', ['status' => 'completed']) }}" 
                   class="px-8 py-3 rounded-2xl font-bold text-sm transition-all {{ request('status') === 'completed' ? 'bg-[#C82333] text-white shadow-xl shadow-red-100' : 'bg-white text-gray-500' }}">
                    Completed
                </a>
            </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div class="flex-1">
                            <div class="flex items-center gap-4 mb-4">
                                <h3 class="text-2xl font-black text-[#1A1C1E]">{{ $mission->title }}</h3>
                                <span class="bg-blue-50 text-blue-600 px-5 py-1.5 rounded-full text-[11px] font-black uppercase">{{ $mission->status }}</span>
                                <span class="bg-green-50 text-green-600 px-5 py-1.5 rounded-full text-[11px] font-black uppercase font-bold">RM {{ number_format($mission->commission, 0) }}</span>
                            </div>
        ...
        <div class="bg-white rounded-[2rem] max-w-2xl w-full shadow-2xl overflow-hidden">
            ...
            <div class="px-8 py-8 flex justify-between items-start">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        <h2 id="m-title" class="text-2xl font-black text-gray-900"></h2>
            ...
            <div class="px-8 flex gap-3 mb-8">
                <span id="m-status-badge" class="bg-blue-50 text-blue-600 px-6 py-2 rounded-full text-xs font-black uppercase"></span>
                <span class="bg-green-50 text-green-600 px-6 py-2 rounded-full text-xs font-black flex items-center gap-2">
        ...
        <div class="bg-white rounded-[2rem] max-w-2xl w-full shadow-2xl overflow-hidden">
            <div class="px-10 py-8 flex justify-between items-center border-b">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-2xl font-black">Create New Task</h3>
                </div>
